...
Elm.Todo.make = function (_elm) {
   ...
   var TimeStep = function (a) {
      return {ctor: "TimeStep"
             ,_0: a};
   };
   var timeSig = A2($Signal.map,
   TimeStep,
   $Time.every($Time.second));
   var Clicky = function (a) {
      return {ctor: "Clicky"
             ,_0: a};
   };
   ...
   var saleToRow = function (sale) {
      ...
      _L.fromArray([A2($Html.td,
                   _L.fromArray([]),
                   _L.fromArray([$Html.text($Basics.toString(sale.price))]))
                   ,A2($Html.td,
                   _L.fromArray([]),
                   _L.fromArray([$Html.text($Basics.toString(sale.timeLeft))]))]));
   };
   var salesToRows = function (sales) {
      return A2($Html.tbody,
      _L.fromArray([]),
      A2($List.map,saleToRow,sales));
   };
   var salesTable = function (model) {
      return A2($Html.section,
      _L.fromArray([$Html$Attributes.$class("entry")
                   ,$Html$Attributes.id("sales")]),
      ...
      _L.fromArray([A2($Html.caption,
                   ...
                   _L.fromArray([A2($Html.td,
                                ...
                                ,A2($Html.td,
                                _L.fromArray([]),
                                _L.fromArray([$Html.text("Seconds Left")]))]))]))
                   ,salesToRows(model.saleList)]))]));
   };
   var stateEntry = function (model) {
      return A2($Html.section,
      _L.fromArray([$Html$Attributes.$class("entry")
                   ,$Html$Attributes.id("states")]),
      _L.fromArray([A2($Html.p,
                   _L.fromArray([]),
                   _L.fromArray([$Html.text(A2($Basics._op["++"],
                   "Price: ",
                   $Basics.toString(model.price)))]))
                   ,A2($Html.p,
                   _L.fromArray([]),
                   _L.fromArray([$Html.text(A2($Basics._op["++"],
                   "Seconds Remaining: ",
                   $Basics.toString(model.timeLeft)))]))
                   ,A2($Html.p,
                   _L.fromArray([]),
                   _L.fromArray([$Html.text(A2($Basics._op["++"],
                   "Tickets Remaining: ",
   ...
   var inputEntry = function (model) {
      return A2($Html.section,
      _L.fromArray([$Html$Attributes.$class("entry")
                   ,$Html$Attributes.id("inputs")]),
      _L.fromArray([A2($Html.p,
      _L.fromArray([$Html$Attributes.id("myP")]),
      _L.fromArray([$Html.text(instructions)]))]));
   };
   var makePrice = function (model) {
      return function () {
         var b = $Basics.toFloat(model.minP);
         var a = $Basics.toFloat(model.maxP);
         var iu = $Basics.toFloat(model.sales);
         var it = $Basics.toFloat(model.tickets);
         var i$ = iu / it;
         var tu = $Basics.toFloat(model.totalTime - model.timeLeft);
         var tt = $Basics.toFloat(model.totalTime);
         var t$ = tu / tt;
         var v = t$ - i$;
         var v$ = (v + 1) / 2;
         var fp = b * v$ + a * (1 - v$);
         return $Basics.round(fp);
      }();
   };
   var canPurchase = function (model) {
      return _U.cmp(model.tickets - model.sales,
      0) > 0 && _U.cmp(model.timeLeft,
      0) > 0;
   };
   ...
   $default) {
      return function () {
         var res = $String.toInt(str);
         return function () {
            switch (res.ctor)
            {case "Err": return $default;
               case "Ok": return res._0;}
            _U.badCase($moduleName,
            "between lines 137 and 139");
         }();
      }();
   });
   var mergeModels = F2(function (old,
   $new) {
      return function () {
         var newLength = A2(toNumber,
         old.lengthS,
         60);
         var newTickets = A2(toNumber,
         old.ticketsS,
         10);
         var newMax$ = A2(toNumber,
         old.maxS,
         100);
         var newMin = A2(toNumber,
         old.minS,
         0);
         var newMax = _U.cmp(newMax$,
         newMin) < 0 ? newMin : newMax$;
         return _U.replace([["minS"
                            ,$Basics.toString(newMin)]
                           ,["maxS"
                            ,$Basics.toString(newMax)]
                           ,["ticketsS"
                            ,$Basics.toString(newTickets)]
                           ,["lengthS"
                            ,$Basics.toString(newLength)]
                           ,["minP",newMin]
                           ,["maxP",newMax]
                           ,["ticketsP",newTickets]
                           ,["lengthP",newLength]
                           ,["price"
                            ,$Basics.round(($Basics.toFloat(newMin) + $Basics.toFloat(newMax)) / 2)]
                           ,["timeLeft",newLength]
                           ,["totalTime",newLength]
                           ,["tickets",newTickets]],
         $new);
   ...
   var MakePurchase = {ctor: "MakePurchase"};
   var NoOp = {ctor: "NoOp"};
   var updates = $Signal.channel(NoOp);
   var inputCreator = F2(function (desc,
   str) {
      ...
      _L.fromArray([$Html$Attributes.value(str)
                   ...
                   function ($) {
                      return $Signal.send(updates)(UpdateS(desc)($));
                   })]),
      _L.fromArray([]));
   });
   var inputForm = function (model) {
      return A2($Html.section,
      _L.fromArray([$Html$Attributes.$class("entry")
                   ,$Html$Attributes.id("inputs")]),
      ...
      _L.fromArray([A2($Html.label,
                   _L.fromArray([]),
                   _L.fromArray([$Html.text("Min Price")]))
                   ,A2(inputCreator,
                   "Min Price",
                   model.minS)
                   ,A2($Html.label,
                   _L.fromArray([]),
                   _L.fromArray([$Html.text("Max Price")]))
                   ,A2(inputCreator,
                   "Max Price",
                   model.maxS)
                   ,A2($Html.label,
                   ...
                   ,A2($Html.label,
                   _L.fromArray([]),
                   _L.fromArray([$Html.text("Seconds")]))
                   ,A2(inputCreator,
                   "Seconds",
                   model.lengthS)]))]));
   };
   var buttonEntry = function (model) {
      return A2($Html.section,
      _L.fromArray([$Html$Attributes.$class("entry")
   ...
   var view = function (model) {
      return A2($Html.div,
      _L.fromArray([$Html$Attributes.$class("global")]),
      _L.fromArray([A2($Html.div,
                   _L.fromArray([]),
                   _L.fromArray([A2($Html.section,
                   _L.fromArray([$Html$Attributes.id("todoapp")]),
                   _L.fromArray([myHeader]))]))
                   ,A2($Html.div,
                   _L.fromArray([$Html$Attributes.$class("div-class")
            ...
            return function () {
                 ...
                 {case "Max Price":
                    ...
                    return _U.replace([["minS"
                      ...
                      model);
                    case "Seconds":
                    return _U.replace([["lengthS"
                                       ,action._1]],
                      model);
                    case "Tickets":
                    return _U.replace([["ticketsS"
                                       ,action._1]],
                      model);}
                 _U.badCase($moduleName,
                 "between lines 122 and 132");
              }();}
         _U.badCase($moduleName,
         "between lines 105 and 132");
      }();
   });
   var processInput = F2(function (input,
   model) {
      return function () {
         switch (input.ctor)
         {case "Clicky":
            return A2(processAction,
              input._0,
              model);
            case "TimeStep":
            return A2(processTime,
              input._0,
              model);}
         _U.badCase($moduleName,
